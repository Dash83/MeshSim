...
/// This test is designed to evaluate the happy path of AODVDA.
#[test]
fn aodvda_basic() {
    let test_name = String::from("aodvda_basic");
    ...
    //25 processes are expected because there are 25 nodes in the test.
    ...
    ///Obtain the log records for node25 from file <work_directory_path>/log/node25.log
    ...
    // Assert that node 25 received 2 packets.
    // The test performs of 2 actions, where 2 different nodes send a DATA packet to node25.
...
/// This test is designed to evaluate the route error detecting capabilities of AODVDA.
...
#[test]
fn aodvda_rerr() {
    let test_name = String::from("aodvda_rerr");
    let data = setup(&test_name, false, false);
