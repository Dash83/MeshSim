...
impl Protocol for ReactiveGossipRoutingIII {
    fn handle_message(
        ...
        ReactiveGossipRoutingIII::handle_message_internal(
            hdr,
            msg,
            self_peer,
            msg_id,
    ...
    fn init_protocol(&self) -> Result<Option<MessageHeader>, MeshSimError> {
        ...
        let sr_radio = Arc::clone(&self.short_radio);
        let lr_radio = Arc::clone(&self.long_radio);
        let data_msg_cache = Arc::clone(&self.data_msg_cache);
        let dest_routes = Arc::clone(&self.destination_routes);
        let known_routes = Arc::clone(&self.known_routes);
...
impl ReactiveGossipRoutingIII {
    ...
    fn handle_message_internal(
        ...
        let _ = ReactiveGossipRoutingIII::update_vicinity_cache(&hdr,
        ...
        match msg {
            ...
            Messages::Beacon => {
                ReactiveGossipRoutingIII::process_beacon_msg(
                    hdr,
                    vicinity_cache,
                    logger
                )
    ...
    fn process_data_msg(
...

        {
            ...
            if let Some((msg, mut entry)) = d_cache.remove_entry(&msg_id) {
                match entry.state {
    ...
    fn process_route_discovery_msg(
        ...
        if msg.route_destination == self_peer.name {
            ...
            hdr.sender = self_peer;
            let response_payload = serialize_message(Messages::RouteEstablish(msg))?;
            debug!(logger, "Response size: {}", response_payload.len());
            hdr.payload = Some(response_payload);

            let resp_id = hdr.get_hdr_hash();
        ...
        //     "status"=>MessageStatus::FORWARDED,
        // );
        let md = MessageMetadata::new(hdr.get_hdr_hash(), "ROUTE_DISCOVERY", MessageStatus::FORWARDING);
        // md.source = Some(&hdr.sender.name);
    ...
    fn process_route_established_msg(
        ...
        //Build the packet metadata
        let mut md = MessageMetadata::new(hdr.get_hdr_hash(), "ROUTE_ESTABLISH", MessageStatus::FORWARDING);
        // md.source = Some(&hdr.sender.name);
        // md.route_id = Some(&msg.route_id);
    ...
    fn process_route_teardown_msg(
        ...
        let (response, md) = {
            if subscribed {
                ...
                // );
                let hdr = ReactiveGossipRoutingIII::route_teardown(
                    &msg.route_id,
                    &self_peer,
                    dest_routes,
                    known_routes,
                ...
                let mut md = MessageMetadata::new(hdr.get_hdr_hash(), "ROUTE_TEARDOWN", MessageStatus::FORWARDING);
                // md.source = Some(&hdr.sender.name);
                // md.route_id = Some(&msg.route_id);
    ...
    fn process_beacon_msg(
        ...
        let mut vc = vicinity_cache
            .lock()
            .expect("Could not lock vicinity cache");
        vc.insert(hdr.sender.name.clone(), Utc::now());
        ...
        if let Some((_key, flows)) = entry {
            ...
            for data in flows {
                ...
                thread_pool.execute(move || {
                    match ReactiveGossipRoutingIII::start_flow(r_id.clone(), dest, s, data, radio, dmc, &l)
                    {
                        Ok(_) => {
                            // All good!
                        }
    ...
    fn maintenance_loop(
            ...
            //Vicinity cache maintenance
	    {
            ...
            //Beacon transmission
            let msg = Messages::Beacon;
            let mut hdr = MessageHeader::new();
            hdr.sender = me.clone();
            hdr.payload = Some(serialize_message(msg)?);
            match long_radio.broadcast(hdr) { 
                Ok(_) => { 
                    info!(
                        logger,
                        "Beacon sent";
