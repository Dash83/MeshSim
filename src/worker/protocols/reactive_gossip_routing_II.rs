...
impl Protocol for ReactiveGossipRoutingII {
    fn handle_message(
        ...
        let resp = ReactiveGossipRoutingII::handle_message_internal(
            hdr,
            msg,
            self_peer,
            msg_id,
    ...
    fn send(&self, destination: String, data: Vec<u8>) -> Result<(), MeshSimError> {
        ...
        if let Some(r_id) = route_id {
            ...
            let log_data = ProtocolMessages::RGRII(msg.clone());
            ...
            ReactiveGossipRoutingII::queue_transmission(qt, destination, hdr)?;
        }
        Ok(())
    }
    ...
    fn do_maintenance(&self) -> Result<(), MeshSimError> {
        if Utc::now().timestamp_nanos() >= self.ts_data_retransmission.load(Ordering::SeqCst) {
            match ReactiveGossipRoutingII::data_retransmission(
                Arc::clone(&self.data_msg_cache),
        ...
        if Utc::now().timestamp_nanos() >= self.ts_rd_retransmission.load(Ordering::SeqCst) {
            match ReactiveGossipRoutingII::process_expired_route_discoveries(
        ...
        if Utc::now().timestamp_nanos() >= self.ts_vinicity_maintenance.load(Ordering::SeqCst) {
            match ReactiveGossipRoutingII::vicinity_cache_maintenance(
...
impl ReactiveGossipRoutingII {
    ...
    fn start_route_discovery(
        ...
        ReactiveGossipRoutingII::update_queued_transmissions(
            queued_transmissions,
            &destination,
            route_id.clone(),
        )?;
    ...
    fn process_data_msg(
        ...
        if hdr.destination == self_peer {
            ...
            let log_data = ProtocolMessages::RGRII(msg);
    ...
    fn process_route_teardown_msg(
        ...
        self_peer: String,
        _msg_id: String,
        _wifi_tx_queue: Sender<Transmission>,
        logger: &Logger,
    ) -> Result<HandleMessageOutcome, MeshSimError> {
        let subscribed = {
            let kr = known_routes
    ...
    fn start_queued_flows(
        ...
        if let Some((_key, flows)) = entry {
            ...
            for hdr in flows {
                ...
                let log_data = ProtocolMessages::RGRII(deserialize_message(hdr.get_payload())?);
    ...
    //                     entry.retries += 1;
    //                     let log_data =
    //                         ProtocolMessages::RGRII(deserialize_message(hdr.get_payload())?);
    //                     info!(
    //                         logger,
