...
pub fn teardown(data: TestSetup, delete_db: bool) {
    ...
    if data.db_env_file.is_some() && delete_db {
        ...
        let conn = get_db_connection(&root_env_file, &data.logger).expect("Could not get DB connection for cleanup");
        ...
        let _rows  = q.execute(&conn).expect("Could not execute DROP DATABASE statement");
    }
